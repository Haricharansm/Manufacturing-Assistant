...
if colC.button("Step"):
    ...
    if sc and st.session_state.script_idx < len(sc["events"]):
        e = sc["events"][st.session_state.script_idx]
...
if persona == "Sales AE":
    ...
    if st.sidebar.button("Create expedited PO"):
        assistant(simapi.post_action("Create expedited PO"))
    if st.sidebar.button("Alternate supplier"):
        assistant(simapi.post_action("Trigger alternate supplier"))
    if st.sidebar.button("Upgrade carrier to air"):
        assistant(simapi.post_action("Upgrade carrier to air"))

else:
    if st.sidebar.button("Re-sequence L2"):
        assistant(simapi.post_action("Re-sequence L2"))
    if st.sidebar.button("Batch changeovers"):
        assistant(simapi.post_action("Batch changeovers"))
    if st.sidebar.button("QA fast-track"):
        assistant(simapi.post_action("QA fast-track"))
